...
namespace VLB
{
    public abstract class VolumetricLightBeamAbstractBase : MonoBehaviour
    {
        ...
        //public Bounds bounds { get { return GetBeamGeometry() != null ? GetBeamGeometry().meshRenderer.bounds : new UnityEngine.Bounds(Vector3.zero, Vector3.zero); } }
        public UnityEngine.Bounds bounds
        {
            get
            {
                var beamGeometry = GetBeamGeometry();
                if (beamGeometry != null && beamGeometry.meshRenderer != null)
                {
                    return beamGeometry.meshRenderer.bounds;
                }
                else
                {
                    // 만약 GetBeamGeometry()가 null을 반환하거나 meshRenderer가 null이면 예외를 던집니다.
                    throw new System.NullReferenceException("Beam geometry or mesh renderer is null.");
                }
            }
        }
